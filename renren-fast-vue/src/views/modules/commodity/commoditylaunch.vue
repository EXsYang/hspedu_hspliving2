  ...
  methods: {
    ...
    getCatBrands() {
      this.$http({
        ...
        // console.log("获取到某个分类的品牌 data-> ", data)
        if (data != null) {
        }
    ...
    getShowSaleAttr() { //获取销售属性
      // console.log("getShowSaleAttr this.spu.catalogId=", this.spu.catalogId)
      //获取当前分类可以使用的销售属性
      if (!this.dataResp.steped[1]) {
        this.$http({
          ...
          // console.log("getShowSaleAttr data==> ",data)
    ...
    showBaseAttrs() {//得到当前分类的基本属性
      if (!this.dataResp.steped[0]) {
        this.$http({
          ...
          //先对表单的baseAttrs进行初始化
          // console.log("showBaseAttrs data=> ", data)

          data.data.forEach(item => {
            let attrArray = [];

            // 如果item.attrs不为null才进入遍历
            // if (item.attrs != null) {
