...
export default {
  ...
  mounted() {
    //订阅事件：在第二个组件中（假设为组件B），通过调用 PubSub.subscribe("dataForm", (msg, val) => { this.dataForm = val; this.getDataList(); }); 订阅了 "dataForm" 事件。这意味着每当 "dataForm" 事件被发布时，该匿名函数会被执行，将接收到的数据赋值给 dataForm 并调用 getDataList() 方法。
