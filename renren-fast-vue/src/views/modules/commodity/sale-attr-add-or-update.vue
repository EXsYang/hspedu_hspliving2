...
export default {
  ...
  methods: {
    ...
    init(id) {
      ...
      // 的属性确定的属性名
      //|| (逻辑或运算符): 这个运算符用来连接两个表达式，它会从左到右计算这两个表达式。
      // 如果左边的表达式 (id) 可以转换为 true（在 JavaScript 中称为“truthy”）
      // ，则返回左边表达式的值；如果左边的表达式可以转换为 false
      // （在 JavaScript 中称为“falsy”），则返回右边表达式的值。
      this.dataForm.attrId = id || 0

      // 如果是新增则将cascadedCategoryId清空
      if (!id){
        console.log("是新增~")
